...
class DataLink(object):
    ...
    def __init__(
        ...
        if self._stop_time <= self._start_time:
            log.error(
                f"DataLink: trying to create a DataLink to a {self._segment_type:s} with the stop time {self._stop_time} less or equal to the {self._start_time}!"
            )
            raise ValueError(
                f"DataLink: trying to create a DataLink to a {self._segment_type:s} with the stop time {self._stop_time} less or equal to the {self._start_time}!"
            )
    ...
    def from_pandas(data_frame: pd.DataFrame, index: int):
        ...
        if index not in data_frame.index:
            log.error(f"from_pandas: index {index} is invalid in data_frame")
            return None
        specs = {}
        ...
        for c in DataLink.columns():
            if c not in data_frame.columns:
                log.error(
                    f"DataLink.from_pandas: required column {c} not in data frame!"
                )
                raise ValueError(
                    f"DataLink.from_pandas: required column {c} not in data frame!"
                )
...
def from_pandas(
    ...
    if index is None:
        if segment_type is not None:
            ...
            return [
                DataLink.from_pandas(data_frame, index) for index in data_frame.index
            ]
    else:
        return DataLink.from_pandas(data_frame, index)


def load_data_segment(
    data_link: DataLink, trace_name: str, before=0.0, after=0.0, data_location="."
    ...
    if not os.path.exists(converted_path):
        log.error(f"Nix file {filename} could not be read from path {converted_path}!")
        return None, None

    nf = nixio.File.open(converted_path, nixio.FileMode.ReadOnly)
    if data_link.block_id not in nf.blocks:
        log.error(f"Block with id {data_link.block_id} is not found in {filename}!")
        return None, None
    ...
    if SegmentType[data_link.segment_type] is SegmentType.ReproRun:
        if data_link.tag_id not in block.tags:
            log.error(f"Tag with id {data_link.tag_id} is not found in {block}!")
            return None, None
        ...
        if data_link.tag_id not in block.multi_tags:
            log.error(f"MultiTag with id {data_link.tag_id} is not found in {block}!")
            return None, None
        tag = block.multi_tags[data_link.tag_id]
    else:
        log.error(
            f"load_data_segment: Segment type ({data_link.segment_type}) is invalid! Allowed values are {SegmentType.StimulusSegment} or {SegmentType.ReproRun}!"
        )
        return None, None

    if trace_name not in tag.references:
        log.error(
            f"The given tag does not refer to a trace {trace_name}! Or a trace with that name does not exist (traces are: {tag.references})!"
        )
        return None, None
    ...
    if (event_type not in t) and (continuous_type not in t):
        log.error(
            f"Can only process event ({event_type}) or continuous ({continuous_type}) data! Found type: {data_array.type}!"
        )
        return None, None
    ...
    if before > data_link.max_before:
        log.warning(
            f"The given before time {before} exceeds the maximum valid before time {data_link.max_before}. Set to maximum valid time!"
        )
    ...
    if after > data_link.max_after:
        log.warning(
            f"The given after time {before} exceeds the maximum valid after time {data_link.max_after}. Set to maximum valid time!"
        )
    ...
    log.info(
        f"Reading data from data array {data_array} in the interval {start_time}, {stop_time}."
    )
